...
_TOOLCHAIN_SECTION_HINTS = ("dev", "lint", "test", "ci", "typecheck", "quality", "check", "style")

_KNOWN_CI_TOOLS = frozenset({
    "ruff", "mypy", "black", "flake8", "pylint", "isort", "pyright",
    "pyflakes", "pycodestyle", "bandit", "pyupgrade", "autoflake", "yapf",
    "pydocstyle", "vulture", "ssort", "docformatter", "flake8-bugbear",
})
...
_OPTDEP_TABLE = re.compile(r"^\s*\[(?:project\.optional-dependencies|dependency-groups)\]")
...
def _is_floor_only(spec: str) -> bool:
    return bool(_HAS_FLOOR.search(spec)) and not _HAS_CEILING.search(spec)


def _section_is_toolchain(name: str) -> bool:
    n = name.lower()
    return any(h in n for h in _TOOLCHAIN_SECTION_HINTS)
...
class UnpinnedToolchainAnalyzer:
    ...
    def _scan_toml_like(self, text: str, rel: str) -> list[Finding]:
        ...
        for lineno, raw in enumerate(text.splitlines(), start=1):
            ...
            if m_open and in_optdep_table:
                ...
                if not in_array and _section_is_toolchain(grp):
    ...
    def _flag_line(
        ...
        for m in _DEP_SPEC.finditer(line):
            ...
            spec = (m.group(2) or "").strip().rstrip(",").strip()
            if not _is_floor_only(spec):
                continue
            known = tool in _KNOWN_CI_TOOLS
            out.append(
                self._finding(rel, lineno, tool, spec, line.strip()[:80], section, known)
            )
        return out
