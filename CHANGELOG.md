...
- `canonical_pill` **1.0.0** (advisory, CANON-PILL-001): detect **unsourced imperative prohibitions** in canonical governance / spec Markdown — a permanent rule (`NEVER`, `MUST NOT`, `LOCKED`, …) with no citation anywhere in its section. The failure mode is a descriptive observation frozen into permanent law, locking out a still-developmental capability. KEEP/OVERREACH split is *citation, not rationale*: a prohibition backed by a reference, standard, tolerance or version pin is silent; one backed only by a prose reason is a candidate. Section-scoped citation adjacency; case-sensitive uppercase tokens (canonical prohibitions are shouted, casual prose isn't); PRIMARY requires canonical register (prescriptive heading or doc-level governance status marker). Intra-doc contradictions are attached as `contradiction_refs` metadata only — never a severity judgment. All findings are CANDIDATE / `context_confirmed=False`: the analyzer surfaces leads, it does not rule on them. Suite severity LOW (a candidate must never fail a build); the MEDIUM/HIGH ladder is carried by `confidence` and `candidate_strength`. Markdown-only in v1 — the code form of over-strict canonicalization (`extra="forbid"` schemas, frozen invariants) is the same class but a separate, deliberately unbuilt detector.
