...
- `unguarded_stdout_encoding` **1.0.0** (advisory, DETECT-PROMOTE-001A): detect CLI entry modules (`__main__.py`, an `if __name__ == "__main__":` block, or argparse/click/typer + `main()`) that write non-ASCII to stdout/stderr with no UTF-8 guard. On Windows (Python < 3.15) a piped, redirected or captured stream uses the locale codepage (cp1252); each finding records which of three failure modes applies: `encode_crash` (stdout, character outside cp1252), `escaped_output` (stderr, `backslashreplace`) and `undecodable_bytes` (a cp1252-encodable character such as an em-dash, unreadable to a UTF-8 reader). Two evidence tiers: *high* when a non-ASCII literal reaches the sink through at most two local bindings; *low* when an unguarded sink prints an untraceable value and the module holds non-ASCII constants (the scaffolder `print(content)` shape). Guards are per-stream (`reconfigure(encoding="utf-8")`, including the loop form, or a UTF-8 `TextIOWrapper` rebind); `PYTHONIOENCODING` set in-process is not a guard. `guard_confirmed=False` leaves invocation-level guards to a v2 pass. Family II (implicit context); a standalone analyzer rather than a `context_pinned_hash` axis, because every axis there is scoped to a value that reaches a hash. Suite severity LOW; registered in the default analyzer set.
...
- `namespace_authority_drift` **0.2.1** (advisory, CAT-NAD-001B): path-string-only `str` contract (raw JSON strings rejected), `Mapping` accepted consistently across loader/analyzer/`scan_project`, dedicated `code-audit namespace-authority <root> --context <file> [--json]` CLI with advisory exit (findings → 0; malformed/missing → 2), and contract edge-case tests (schema dual-copy parity, source_registry metadata-only, deterministic ordering, loader failures).
- `namespace_authority_drift` **0.2.0** (advisory, CAT-NAD-001B): strict `namespace_authority_context_v1` schema + loader for serialized Path/JSON activation; direct `NamespaceAuthorityContext` still accepted; legacy object-bearing dicts rejected by schema. Envelope schema dual-copied under `schemas/` and `data/schemas/` with byte parity.
