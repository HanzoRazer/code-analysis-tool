...
- `deployment` **1.1.0**: new `watch_coverage` validator (registered + enabled by default) — flags trigger/coverage sets narrower than what the build actually consumes (Family IV; distinct from `hollow_guarantee`, where the guard cannot fire — here it can fire but its input set is too narrow). Three checks: GitHub Actions `on.*.paths` filters that omit a script the job runs or a `package-lock.json`/`.nvmrc` a Node install reads; Railway `watchPatterns` that omit a `startCommand`/`buildCommand` file; and Dockerfiles present in the repo that no CI job builds. Findings are MEDIUM. Logic manifest refreshed; version bumped. Self-scan caught a real gap in this repo's own CI (`copy-lint.yml` runs `scripts/locale_parity.py`, which its path filter does not watch). Pure helpers kept at module scope and the test class split so the analyzer adds zero new structural debt (ratchet baseline untouched).
