...
- `unbacked_claim` **1.0.0**: detect a comment/docstring that claims a library the module never imports — the claimed code path cannot exist (the purest failure-mimics-success shape: a capability claim in front of a same-shaped substitute). HIGH when the claim is a fallback / if-available shape (silent substitution), MEDIUM for a plain "uses X" / "X-based" claim; a mere prose mention ("faster than scipy") is not a claim and stays silent — the claim shape is the discriminator, not the bare name. Backed claims (the library imported anywhere in the module, including try/except-guarded, function-local, or `importlib.import_module`) do not fire. Known heavy/optional libs only (scipy/numba/torch/…); ubiquitous ones (numpy/pandas) are excluded to keep the signal clean. `AnalyzerType.UNBACKED_CLAIM` added; registered in the default set; confidence_golden/confidence_policy/logic manifests refreshed for the new enum member. Pure helpers at module scope so the analyzer adds zero new structural debt. Born-from-the-bug fixture: MAINT-DEFER-010's `rayleigh_ritz.py` (`# Fallback to scipy if available` with no scipy import, running a `np.diag` substitute).
