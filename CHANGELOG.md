...
- `dangling_reference` **1.0.0**: detect references to first-party modules that no longer exist in the tree — the inverse of `dead_code` (callee deleted, callers left in place; together the two halves of deletion safety). Resolves at module granularity across Python `import`/`from … import` and unambiguous config contexts (`python -m`, inline `import`, TOML entry-points in shell/YAML/Makefile/TOML). The discriminator is portable: fire only when a reference's *top-level* package still resolves but its *full dotted path* does not, which naturally excludes missing third-party imports. `AnalyzerType.DANGLING_REFERENCE` added; registered in the default analyzer set; `confidence_golden`/`confidence_policy`/`logic` manifests refreshed for the new enum member (no confidence-scoring change; matches the `order_dependence`/`hollow_guarantee` precedent). Symbol-level resolution (module survives, a name inside it deleted) is deferred, mirroring dead_code's single-granularity v1. Born-from-the-bug fixture: the MAINT-DEFER-009 `app/retopo/` deletion whose `run.sh` still calls `python -m app.retopo.run`.
