...
- `hollow_guarantee` **1.0.2** / `maxfail_masking` **2.0.1** (advisory): bank the Family V silencer protocol in the fix-hint — "removing this silencer will likely expose a real failure it was hiding; the fix is remove the silencer AND fix what surfaces, not just remove the silencer." `exceptions` carries the same note at its module docstring rather than in finding output, because its swallow message is pinned by the signal_logic golden fixtures and a cosmetic remediation note is not a signal-logic change (so no `signal_logic_version` bump). Logic manifest refreshed; no structural-debt or golden-fixture drift.
