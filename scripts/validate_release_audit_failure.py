"""Validate a release audit failure JSON file against its schema.
...
SCHEMA_PATH = ROOT / "dist" / "release_audit_failure.schema.json"


def main() -> int:
    if len(sys.argv) < 2:
        print(f"Usage: {sys.argv[0]} <audit_failure.json>", file=sys.stderr)
        return 2
    ...
    if not errors:
        print(f"[validate-audit-failure] OK: {target_path} is valid.")
        return 0

    print(f"[validate-audit-failure] FAIL: {len(errors)} validation error(s).")
