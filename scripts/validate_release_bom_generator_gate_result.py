"""Validate a BOM generator gate result JSON file against its schema.

Usage:
    python scripts/validate_release_bom_generator_gate_result.py result.json
"""
...
SCHEMA = ROOT / "schemas" / "release_bom_generator_gate_result.schema.json"
...
def main() -> int:
    ...
    if not errors:
        print(f"[validate-bom-generator-gate-result] OK: {target_path} is valid.")
        return 0

    print(f"[validate-bom-generator-gate-result] FAIL: {len(errors)} validation error(s).")
